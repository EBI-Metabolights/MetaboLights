package uk.ac.ebi.metabolights.repository.model.studyvalidator;

/**
 * Created by kalai on 18/09/15.
 */
public enum ValidationIdentifier {
    STUDY_TITLE(1),
    STUDY_DESCRIPTION(2),
    STUDY_TEXT(3),
    STUDY_CONTACT_EMAIL(4),
    FACTOR_NAME(11),
    FACTOR_IN_SAMPLES_ASSAYS(12),
    FACTOR_COLUMNS_SAMPLES_ASSAYS(13),
    ORGANISM_NAME(21),
    ORGANISM_PART(22),
    PUBLICATIONS(31),
    PUBLICATION_TITLE(32),
    PUBLICATION_AUTHORS(33),
    PUBLICATION_IDS(34),
    PUBLICATION_DOI(35),
    PUBLICATION_IDS_DOI(36),
    PROTOCOLS_ALL(41),
    PROTOCOLS_MINIMUM(42),
    PROTOCOLS_SAMPLE_COLLECTION(43),
    PROTOCOLS_TEXT(44),
    PROTOCOLS_EXTRACTION(45),
    PROTOCOLS_DATA_EXTRACTION(46),
    MAF_PROTOCOLS(47),
    MS_PROTOCOLS_MASS_SPEC(47),
    MS_PROTOCOLS_CROMA(48),
    NMR_PROTOCOLS_NMR_SAMPLE(49),
    NMR_PROTOCOLS_SPECTROSCOPY(50),
    NMR_PROTOCOLS_NMR_ASSAY(51),
    PROTOCOLS_PREPARATION(52),
    PROTOCOLS_HISTOLOGY(53),
    NMR_PROTOCOLS_NMR_MRI(54),
    NMR_PROTOCOLS_IN_VIVO_MRS(55),
    NMR_PROTOCOLS_NMR_IN_VIVO_ASSAY(56),
    ASSAYS(61),
    ASSAY_PLATFORM(62),
    ASSAY_FILES(63),
    ASSAY_FILES_IN_FILESYSTEM(64),
    ASSAY_ALL_MAF_REFERENCE(65),
    ASSAY_ATLEAST_SOME_MAF_REFERENCE(66),
    ASSAYS_EMPTY_COLUMNS(67),
    ASSAY_CORRECT_RAW_FILE(68),
    ASSAY_MAF_FILE(81),
    ASSAY_CORRECT_MAF_FILE(82),
    SAMPLES(101),
    SAMPLES_EMPTY_COLUMNS(102),
    ASSAY_SAMPLE_NAMEMATCH(103),
    ISATAB_INVESTIGATION(121),
    MAF_FILE_CONTENT(122),
    MI_PROTOCOL_CONTENT(123);

    private final int id;

    ValidationIdentifier(final int validationID) {
        id = validationID;
    }

    public int getID() {
        return id;
    }

}
