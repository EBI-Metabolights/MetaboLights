...
-- cursor for MTBLC records
  ...
  -- Ontology, not used!!
  cursor ontology_c(o_study_id NUMBER)  IS
    select distinct rs.ACC as ontology, description
    from 
      ontology_entry oe,
      reference_source rs,
      assay a
    where oe.source = rs.id
      and a.technology = oe.id
    ...
    FOR study_cur IN accession_c LOOP
      ...
      -- First "single" entries from the study
        ...
        ELSE  -- MTBLC (compound) information
          ...
          -- Study design loop
          FOR add_cur IN additional_c(study_cur.study_id) LOOP
            IF ( UPPER(add_cur.design) like 'MASS SPECTROMETRY' OR UPPER(add_cur.design) like 'NMR SPECTROSCOPY' ) THEN
              dbms_output.put_line(replace(replace(add_fields_entry,'FIELD_NAME','technology_type'),'FIELD_VALUE',add_cur.design) );
              add_fields_entry := l_add_fields_entry;
            END IF;
          END LOOP;
