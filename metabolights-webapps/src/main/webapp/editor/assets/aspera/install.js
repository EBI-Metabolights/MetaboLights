...
var installPlugin = function () {
  var downloadMessage, progressContainer;
  // Precondition: plugin is not installed
  if (!awInstaller.installerUpdateAvailable()) return;

  logger("Insert progress bar...");
  downloadMessage = connectInstaller.installPackage
    ? '<br><br><span class="alt_download">If nothing happens, download and run the <a href="' +
      (connectInstaller.manualInstallPackage ||
        connectInstaller.installPackage) +
      '">Aspera Connect installer</a>.<span>'
    : "";
  ...
  awInstaller.setCallback(function (state, desc, percentage) {
    logger(
      "installer prep callback - " + state + ", " + desc + ", " + percentage
    );
...
var installConnect = function (elementId) {
  ...
  header.innerHTML =
    "Installing Aspera Connect v" + connectInstaller.getLatestConnectVersion();
...
var showConnectInstallButton = function () {
  ...
  installButton.innerHTML = connectInstaller.isUpgrade()
    ? "Upgrade Aspera Connect"
  ...
  if (progressContainer) {
    ...
    switch (state.toLowerCase()) {
      ...
      case "error_admin_rights":
        descriptionText =
          AW.utils.localize("ErrAdminRights") +
          " " +
          '<a href="' +
          (connectInstaller.manualInstallPackage ||
            connectInstaller.installPackage) +
          '">' +
          AW.utils.localize("Download") +
          "</a>";
        percentage = 0;
        progressBar.setAttribute("data-error", "true");
        break;
...
var insertRestartButton = function () {
  ...
  restartButton.onclick = function () {
    connectInstaller.restartBrowser();
  };
  container.innerHTML = "";
...
var initAsperaConnect = function () {
  ...
  if (
    connectInstaller.asperaWebInstalled() &&
    !connectInstaller.updateAvailable()
  ) {
