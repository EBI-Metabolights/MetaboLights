...
var FtpSizeExceededMsg =
  "The maximum size of the data for the HTTP tarball download is: " +
  SizeLimitSelectedFTP +
  " B." +
...
function moveAsperaIframe() {
  ...
  document
    ...
    .insertBefore(asperaIframe, document.getElementById("wrapper"));
}
...
METABOLIGHTS.FileControl.prototype.init = function (params) {
  ...
  var CONNECT_AUTOINSTALL_LOCATION =
  ...
  self.connectInstaller = new AW4.ConnectInstaller({
    sdkLocation: CONNECT_AUTOINSTALL_LOCATION,
  });
  ...
  self.asperaWeb.addEventListener(
  ...
  self.asperaWeb.addEventListener("transfer", function (event, obj) {
    self.handleTransferEvents(
      event,
      obj,
      params.transferContainer,
      params.entryName
    );
  });
...
METABOLIGHTS.FileControl.prototype.showHideTransferComponents = function (
  params
) {
  ...
  if (
    !params.transferInfoDiv &&
    !params.progressBar &&
    !params.progressBarTextDiv &&
    !params.transferPauseButton &&
    !params.transferResumeButton &&
    !params.transferAbortButton
  ) {
...
METABOLIGHTS.FileControl.prototype.handleTransferEvents = function (
  event,
  allTransfers,
  divId,
  entryName
  ...
  if (obj) {
    ...
    if (!this.transferPauseButton) {
      ...
      var transferPauseButtonClick = function (e, fcObj) {
        if (
          ...
          (fcObj.transferStatus === "initiating" ||
            fcObj.transferStatus === "running")
        ) {
      ...
      this.transferPauseButton.on("click", function (e) {
        transferPauseButtonClick(e, self);
      });
    }
    ...
    if (!this.transferResumeButton) {
      ...
      this.transferResumeButton.on("click", function (e) {
        transferResumeButtonClick(e, self);
      });
    }
    ...
    if (!this.transferAbortButton) {
      ...
      this.transferAbortButton.on("click", function (e) {
        transferAbortButtonClick(e, self);
      });
    }

    // Hide and show components depending on current status
    if (["running", "queued", "willretry"].indexOf(self.transferStatus) >= 0) {
      this.showHideTransferComponents({
        transferInfoDiv: 1,
        progressBar: 1,
        progressBarTextDiv: 1,
        transferPauseButton: 1,
        transferResumeButton: 0,
        transferAbortButton: 1,
      });
    } else if (self.transferStatus === "initiating") {
      this.showHideTransferComponents({
        transferInfoDiv: 1,
        progressBar: 0,
        progressBarTextDiv: 0,
        transferPauseButton: 1,
        transferResumeButton: 0,
        transferAbortButton: 1,
      });
      ...
      this.showHideTransferComponents({
        ...
        progressBar: 1,
        progressBarTextDiv: 1,
        transferPauseButton: 0,
        transferResumeButton: 1,
        transferAbortButton: 1,
      });
      ...
      this.showHideTransferComponents({
        ...
        transferPauseButton: 0,
        transferResumeButton: 0,
        transferAbortButton: 0,
      });
      setTimeout(function () {
        $(self.transferDiv).css("display", "none");
        if (UploadStarted) {
          UploadStarted = false;
          if (entryName !== undefined) window.open("edit_imagesets/", "_self");
        }
      }, 12000);
...
METABOLIGHTS.FileControl.prototype.addDownloadButton = function (params) {
    ...
    self.asperaWeb.showSelectFolderDialog({
      success: function (dataTransferObj) {
        if (dataTransferObj.dataTransfer.files[0])
          self.download(
            params.source,
            dataTransferObj.dataTransfer.files[0].name
          );
      },
    });
  };
  ...
  uploadButtonNoAspera.addClass(
    "METABOLIGHTSDownloadButton noAsperaControlElement"
  );
...
METABOLIGHTS.FileControl.prototype.download = function (source, path) {
  ...
  if (path != null && path.length > 0) {
    ...
    requestId = self.asperaWeb.startTransfers(
      { transfer_specs: transferSpecs },
      { error: self.handleStartResponse }
    );
  }
};
...
METABOLIGHTS.FileControl.prototype.addUploadButton = function (params) {
  ...
  $("#" + params.uploadContainer).append(
    '<div id="' + params.buttonId + "Aspera" + 'Div"></div>'
  );
  ...
  uploadButton.addClass(
    "METABOLIGHTSDownloadButton asperaControlElement icon icon-functional"
  );
  ...
  var uploadButtonClick = function (e) {
    self.asperaWeb.showSelectFileDialog({
      success: function (dataTransferObj) {
        if (dataTransferObj.dataTransfer.files[0]) {
          self.upload(
            dataTransferObj.dataTransfer.files,
            params.destination,
            self
          );
        }
  ...
  uploadButtonNoAspera.addClass(
    "METABOLIGHTSDownloadButton noAsperaControlElement icon icon-functional"
  );
  ...
  uploadButtonDirNoAspera.addClass(
    "METABOLIGHTSDownloadButton noAsperaControlElement icon icon-functional"
  );
  ...
  uploadButtonDirNoAspera
    .unbind("click")
    .bind("click", uploadButtonDirNoAsperaClick);
};
...
// @param fcObj: METABOLIGHTS.FileControl object
METABOLIGHTS.FileControl.prototype.upload = function (
  paths,
  destination,
  fcObj
) {
  ...
  if (paths != null && paths.length > 0) {
    ...
    requestId = fcObj.asperaWeb.startTransfers(
      { transfer_specs: transferSpecs },
...
METABOLIGHTS.FileControl.prototype.handleStartResponse = function (
  responseData
) {
...
browserTree = function (
  ...
  $(function () {
    var jsTreeCore = {
      data: {
        url: function (node) {
          if (typeof imageSetId == "undefined") imageSetId = "-1";
          return node.id === "#"
            ? entryUrl +
                "/ftpServer=" +
                ftpServer +
            ...
            : entryUrl +
                ...
                dirStruct +
                "&list=" +
                (parseInt(node.id) + parseInt(1)).toString() +
                "/" +
                imageSetId;
        },
        data: function (node) {
  ...
  if (treeFileController != -1) {
    ...
    downloadButtonAspFtp.addClass(
      "METABOLIGHTSDownloadButton icon icon-functional"
    );
    ...
    downloadButtonAspFtp.on("click", function () {
      ...
      if (arr.length < 1) {
        alert(
          "Please select files and/or folders which you would like to download."
        );
        return;
      }

      treeFileController.addDownloadButtonTree({
        buttonId: buttonId,
      ...
      treeFileController.addFtpDownloadButtonTree({
        buttonId: buttonId,
        buttonLabel: "Download",
        downloadContainer: "METABOLIGHTSChooseDlInner",
        source: sourcePath,
...
METABOLIGHTS.FileControl.prototype.addDownloadButtonTree = function (params) {
  ...
  downloadButton.addClass(
    "METABOLIGHTSDownloadButton asperaControlElement icon icon-functional"
  );
  downloadButton.attr("data-icon", "=");
  $("#" + params.downloadContainer).append(
    '<div id="' + params.buttonId + "Aspera" + 'Div"></div>'
  );
  ...
  var downloadButtonClick = function (e) {
    ...
    if (arr.length > 0) {
      ...
      for (var i = 0; i < arrayLength; ++i) {
        parents.push(
          "archive/" + $(params.treeId).jstree("get_path", arr[i], "/")
        );
      }
      ...
      self.asperaWeb.showSelectFolderDialog({
        success: function (dataTransferObj) {
          if (dataTransferObj.dataTransfer.files[0])
            self.download(parents, dataTransferObj.dataTransfer.files[0].name);
        },
      });
    } else
      alert(
  ...
  downloadButtonNoAspera.addClass(
    "METABOLIGHTSDownloadButton noAsperaControlElement icon icon-functional"
  );
  ...
  downloadButtonNoAspera
    .unbind("click")
    .bind("click", downloadButtonNoAsperaClick);
};
  ...
  $.ajax({
    ...
    success: function (dataPy) {
      ...
      if (dataPy == "Ready" || dataPy == "error_ftp") {
        ...
        if (dataPy == "Ready") window.location.assign("gettar=" + params.name);
        // If an error has occurred, redirect to the ftp error page
        else if (dataPy == "error_ftp") window.location.assign("ftp_error");
      }
...
// @param params.downloadContainer: DIV or HTML element in which to put the download button
// @param params.treeId: ID of the jsTree that is being associated with the downlaod button
METABOLIGHTS.FileControl.prototype.addFtpDownloadButtonTree = function (
  params
) {
  var self = this;
  ...
  var ftpDownloadButton = $("<button/>", {
    ...
    text:
      "HTTP tar: suitable only for small data sets (< " +
      SizeLimitSelectedFTP / 1073741824 +
      " GB)",
  });
  ftpDownloadButton.addClass("METABOLIGHTSDownloadButton icon icon-functional");
  ftpDownloadButton.attr("data-icon", "=");
  $("#" + params.downloadContainer).append(
    '<div id="' + params.buttonId + "Ftp" + 'Div"></div>'
  );
  ...
  var ftpDownloadButtonClick = function (e) {
    ...
    if (arr.length > 0) {
      ...
      for (var i = 0; i < arrayLength; ++i) {
        ...
        sizeChecked += parseInt(
          $(params.treeId)
            .jstree(true)
            .get_node($(params.treeId).jstree("get_top_checked")[i]).original
            .size
        );
        parents.push(
          "/archive/" + $(params.treeId).jstree("get_path", arr[i], "/")
      ...
      if (sizeChecked > SizeLimitSelectedFTP) {
        ...
        parents
          .join("|")
          .indexOf("The file with the list of directories does not exist") >= 0
      )
        alert("Please select a valid file.");
      else {
        var name = prompt(
          "Please enter the filename under which you would like to save the tarball:",
          "archive.tar"
        );
        if (name != null) {
          showLoader();
