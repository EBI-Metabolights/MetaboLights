				...
				<div class="grid_24">
					<span class="right">
						...
						<c:forEach var="times" begin="1" end="${NumOfPages}" step="1">
							...
							<!--<input type="hidden" name="PageNumber" value="${PageNumId}"/>
							<c:out value="${PageNumber}">${PageNumber}</c:out>-->
						</c:forEach>
							<!--<c:out value="${NumOfPages}">${NumOfPages}</c:out> 
							NumOfPages: <c:out value="${NumOfPages}">${NumOfPages}</c:out>
			       			RemainderItemes: <c:out value="${RemainderItems}">${RemainderItems}</c:out> -->
					</span>
				</div>
		</div>
			...
			<c:forEach var="entry" items="${entries}">
				...
				<div class="grid_24 refLayerBox">
					...
					<div class="grid_16 omega">
						<div class="grid_24">
							<b>${entry.name}</b> (<a href="http://localhost:8080/metabolights-webapp/${entry.accession}">${entry.accession}</a>)
