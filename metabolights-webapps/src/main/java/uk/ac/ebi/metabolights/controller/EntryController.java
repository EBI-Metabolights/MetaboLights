...
public class EntryController extends AbstractController {
    ...
    public static String getBannerMessage() {
        try {
            final MetabolightsWsClient wsClient = MetabolightsWsClient
                    .getInstance(getMetabolightsPythonWsInternalUrl());
    ...
    @RequestMapping({ "/{studyIdentifier:(?:MTBLS|mtbls|REQ|req|MHD|mhd|MHDT|mhdt).+}/assay/{assayNumber}/maf" })
    public ModelAndView getAltMetabolitesIdentified(@PathVariable("studyIdentifier") final String studyIdentifier,
            @PathVariable("assayNumber") final int assayNumber) {
        return this.getMetabolitesModelAndView(studyIdentifier, assayNumber, null);
    }

    private ModelAndView getMetabolitesModelAndView(final String mtblsId, final int assayNumber,
            final String obfuscationCode) {
        ...
        if (obfuscationCode == null) {
            ...
            response = (RestResponse<MetaboliteAssignment>) wsClient.getMetabolitesByObfuscationCode(obfuscationCode,
                    assayNumber);
        }
    ...
    public static boolean canUserDoThisToStudy(final String studyId, MetabolightsUser user,
            final EntryController.PageActions action) {
        if (user == null) {
            user = LoginController.getLoggedUser();
        }
        if (user.isCurator()) {
