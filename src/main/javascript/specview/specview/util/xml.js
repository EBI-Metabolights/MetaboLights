...
goog.provide('specview.dom.xml');

goog.require('specview.dom');
goog.require('specview.dom.NodeType');
 ...
 * @type {number}
 */
specview.dom.xml.MAX_XML_SIZE_KB = 2 * 1024;  // In kB
 ...
 * @type {number}
 */
specview.dom.xml.MAX_ELEMENT_DEPTH = 256; // Same default as MSXML6.
 ...
 */
specview.dom.xml.createDocument = function(opt_rootTagName, opt_namespaceUri) {
  ...
  if (document.implementation && document.implementation.createDocument) {
    ...
    var doc = specview.dom.xml.createMsXmlDocument_();
    if (doc) {
      if (opt_rootTagName) {
        doc.appendChild(doc.createNode(specview.dom.NodeType.ELEMENT,
                                       opt_rootTagName,
                                       opt_namespaceUri || ''));
      }
      return doc;
 ...
 */
specview.dom.xml.loadXml = function(xml) {
 ...
 */
specview.dom.xml.serialize = function(xml) {
 ...
 */
specview.dom.xml.selectSingleNode = function(node, path) {
  if (typeof node.selectSingleNode != 'undefined') {
    var doc = specview.dom.getOwnerDocument(node);
 ...
 *     matching nodes.
 */
specview.dom.xml.selectNodes = function(node, path) {
 ...
 * @private
 */
specview.dom.xml.createMsXmlDocument_ = function() {
  ...
  if (doc) {
    ...
    try {
      ...
      doc.setProperty('MaxXMLSize', specview.dom.xml.MAX_XML_SIZE_KB);
      doc.setProperty('MaxElementDepth', specview.dom.xml.MAX_ELEMENT_DEPTH);
    } catch (e) {
      // No-op.
    }
  }
