...
public class StudyController extends BasicController {
	...
	@ResponseBody
	public StudyRestResponse getStudyByObfuscationCode(@PathVariable("obfuscationcode") String obfuscationCode)
			throws DAOException {
	...
	@ResponseBody
	public RestResponse<Study> getFullStudyById(@PathVariable("studyIdentifier") String studyIdentifier)
			throws DAOException {
	...
	@ResponseBody
	public RestResponse<Study> getLiteStudyById(@PathVariable("studyIdentifier") String studyIdentifier)
			throws DAOException {
		...
		if (chebiWS == null)
			try {
				...
				chebiWS = new ChebiWebServiceClient(new URL(chebiWSUrl),
						new QName("https://www.ebi.ac.uk/webservices/chebi", "ChebiWebServiceService"));
	...
	@ResponseBody
	public RestResponse<String[]> getAllStudyIdentifiersGoingLive(@PathVariable("days") int numberOfDays)
			throws DAOException {
	...
	// @ResponseBody
	// public RestResponse<ArrayList<String>> makeStudiesPublic() throws
	// DAOException {
	...
	// List<String> studyList =
	// studyDAO.getStudiesToGoLiveList(getUser().getApiToken());
	...
	// if (errors > 0) {
	// response.setMessage("There were " + errors + " errors out of " +
	// studyList.size() + ". Check content for details. Last error is in the error
	// object.");
	// } else {
	// if (studyList.size()== 0){
	...
	// * @return
	// */
	// @RequestMapping(value = "{studyIdentifier:" + METABOLIGHTS_ID_REG_EXP
	// +"}/publicreleasedate", method= RequestMethod.PUT)
	// @ResponseBody
	// public RestResponse<Boolean>
	// updatePublicReleaseDate(@PathVariable("studyIdentifier") String
	// studyIdentifier, @RequestBody Date newPublicReleaseDate) throws Exception {

	// User user = getUser();

	// logger.info("User {} requested to update {} public release date to {}",
	// user.getFullName(),studyIdentifier, newPublicReleaseDate);

	// studyDAO= getStudyDAO();

	// // Update the public release date
	// studyDAO.updateReleaseDate(studyIdentifier, newPublicReleaseDate,
	// user.getApiToken());

	// // NOTE: Using IndexController as a Service..this could be refactored. We
	// could have a Index service and a StudyService.
	...
	// restResponse.setMessage("Public release date for " + studyIdentifier + "
	// updated to " + study.getStudyPublicReleaseDate() );
	...
	// */
	// @RequestMapping(value = "{studyIdentifier:" + METABOLIGHTS_ID_REG_EXP
	// +"}/status", method= RequestMethod.PUT)
	// @ResponseBody
	// public RestResponse<Boolean> updateStatusUrl(@PathVariable("studyIdentifier")
	// String studyIdentifier, @RequestBody LiteStudy.StudyStatus newStatus) {
	...
	// restResponse.setContent(true);
	// restResponse.setMessage("Status for " + studyIdentifier + " updated to " +
	// newStatus );

	// } catch (Exception e) {

	// restResponse.setMessage("Couldn't update status for " + studyIdentifier + ":
	// " + e.getMessage());
	 ...
	 */
	@RequestMapping(value = "{studyIdentifier:" + METABOLIGHTS_ID_REG_EXP
			+ "}/overridevalidations", method = RequestMethod.POST)
	@ResponseBody
	public RestResponse<Boolean> updateCuratorsValidationOverride(
			@PathVariable("studyIdentifier") String studyIdentifier, @RequestBody Validations validations) {
		...
		try {
			...
			restResponse.setMessage(
					"Error applying curator's validations override for " + studyIdentifier + ": " + e.getMessage());
			restResponse.setErr(e);
		}
	...
	@ResponseBody
	public RestResponse<Boolean> studyFeedback(@PathVariable("studyIdentifier") String studyIdentifier,
		...
		try {
			...
			if (feedbackObject.containsKey(studyIdentifier)) {
				...
				// check if user exist
				if (doesUserOwnsTheStudy(user.getUserName(), studyDAO.getStudy(studyIdentifier, user.getApiToken()))
						|| user.isCurator()) {
					...
					if (!feedbackObject.containsKey(studyIdentifier)) {
						...
						if (inputData.containsKey("experience") && inputData.containsKey("comments")) {
							...
							userFeedbackObject.put("created_at",
	...
	// private LiteStudy.StudyStatus updateStatus(String studyIdentifier,
	// LiteStudy.StudyStatus newStatus) throws DAOException, IsaTabException,
	// IndexingFailureException {

	// User user = getUser();

	// logger.info("User {} requested to update {} status to {}",
	// user.getFullName(),studyIdentifier, newStatus.name());

	// studyDAO= getStudyDAO();

	// // Update the status
	// Study study = studyDAO.updateStatus(studyIdentifier, newStatus,
	// user.getApiToken());
	...
	private StudyRestResponse getStudy(String studyIdentifier, boolean includeMAFFiles, String obfuscationCode)
			throws DAOException {
	...
	@ResponseBody
	public RestResponse<MetaboliteAssignment> getMetabolites(@PathVariable("studyIdentifier") String studyIdentifier,
			@PathVariable("assayIndex") String assayIndex) throws DAOException {

		logger.info("Requesting maf file of the assay " + assayIndex + " of the study " + studyIdentifier
				+ " to the webservice");

		// Get the study....
		// TODO: optimize this, since we are loading the whole study to get the MAF file
		// name of one of the assay, and maf file can be loaded having only the maf
	...
	@ResponseBody
	public String getMetabolitesJSON(@PathVariable("studyIdentifier") String studyIdentifier,
			@PathVariable("assayIndex") String assayIndex) throws DAOException {
	...
	@ResponseBody
	public String getMetabolitesJSON(@PathVariable("studyIdentifier") String studyIdentifier,
			@PathVariable("assayIndex") String assayIndex, @PathVariable("fileName") String fileName) {

		logger.info("Requesting maf file " + fileName + " as part of the assay " + assayIndex + " of study "
				+ studyIdentifier + " from the webservice");
	...
	@ResponseBody
	public RestResponse<MetaboliteAssignment> getMetabolitesByObfuscationCode(
			@PathVariable("obfuscationcode") String obfuscationCode, @PathVariable("assayIndex") String assayIndex)
			throws DAOException, IsaTabException {

		logger.info("Requesting maf file of the assay " + assayIndex + " by obfuscationcode " + obfuscationCode
				+ " to the webservice");
		// Get the study....
		// TODO: optimize this, since we are loading the whole study to get the MAF file
		// name of one of the assay, and maf file can be loaded having only the maf
	...
	// @ResponseBody
	// public RestResponse<Boolean> deleteStudy(@PathVariable("studyIdentifier")
	// String studyIdentifier) throws DAOException, IsaTabException,
	// IndexingFailureException, IOException, InterruptedException {

	// User user = getUser();

	// logger.info("User {} requested to delete {}", user.getFullName(),
	// studyIdentifier);
	...
	@RequestMapping(value = "{studyIdentifier:" + METABOLIGHTS_ID_REG_EXP
			+ "}/deleteFiles", method = RequestMethod.POST)
	@ResponseBody
	...
	// */
	// @RequestMapping(value = "{studyIdentifier:" + METABOLIGHTS_ID_REG_EXP
	// +"}/restore", method= RequestMethod.PUT)
	// @ResponseBody
	// public RestResponse<Boolean> restore(@PathVariable("studyIdentifier") String
	// studyIdentifier, @RequestBody String backupIdentifier) throws Exception {

	// User user = getUser();

	// logger.info("User {} requested to restore {} backup of {}",
	// user.getFullName(),backupIdentifier,studyIdentifier);

	// studyDAO= getStudyDAO();

	// // Update the public release date
	// studyDAO.restoreBackup(studyIdentifier, user.getApiToken(),
	// backupIdentifier);
	...
	// RestResponse<Boolean> restResponse = new RestResponse<>();
	// restResponse.setContent(true);
	// restResponse.setMessage("Backup (" + backupIdentifier + ") of " +
	// studyIdentifier + " restored.");
	...
	/**
	 * Create a private upload folder for a Study using the API key, so the user can
	 * upload big files using ftp or Aspera.
	 *
	 * @param studyIdentifier
	 * @return
	 * @throws DAOException
	 ...
	 *               createPrivateFtpFolderOnApiKey(@RequestParam(value =
	 *               "studyIdentifier") String studyIdentifier, HttpServletRequest
	 *               request)
	 ...
	 *               logger.info("requestFtpFolderOnApiKey: User {} has requested a
	 *               private upload folder for the study {}, using token {}",
	 *               user.getUserName(), studyIdentifier, token);
	 * 
	 *               // FTP folder is composed with the study identifier + the
	 *               obfuscation code
	 *               String ftpFolder = studyIdentifier.toLowerCase() + "-" +
	 *               getObfuscationCode(studyIdentifier, user);
	 * 
	 *               // create the folder
	 *               String privateFTPRoot =
	 *               PropertiesUtil.getProperty("privateFTPRoot");
	 *               File uploadFolder = new File(privateFTPRoot + File.separator +
	 *               ftpFolder);
	 *               if (!uploadFolder.isDirectory()) { //The folder does not exists
	 *               FileUtil.createFtpFolder(ftpFolder);
	 * 
	 *               // create raw_files folder
	 *               FileUtil.createFtpFolder(ftpFolder + File.separator +
	 *               "RAW_FILES");
	 * 
	 ...
	 *               String userMessage = generateEmail(studyIdentifier, ftpFolder,
	 *               user);
	 *               }
	 * 
	...
	/**
	 * Create a private upload folder for a Study, so the user can upload big files
	 * using ftp or Aspera.
	 *
	 * @param studyIdentifier
	 * @return
	 ...
	 * @RequestMapping("{studyIdentifier:" + METABOLIGHTS_ID_REG_EXP
	 * +"}/files/requestFtpFolder")
	 * 
	 * @ResponseBody
	 *               public RestResponse<String>
	 *               createPrivateFtpFolder(@PathVariable("studyIdentifier") String
	 *               studyIdentifier)
	 *               throws DAOException, IOException, IsaTabException {
	 * 
	 *               User user = getUser();
	 * 
	 *               logger.info("User {} has requested a private upload folder for
	 *               the study {}", user.getUserName(), studyIdentifier);
	 * 
	 *               // FTP folder is composed with the study identifier + the
	 *               obfuscation code
	 *               String ftpFolder = studyIdentifier.toLowerCase() + "-" +
	 ...
	 *               restResponse.setMessage("Your requested upload folder is being
	 *               created. Details for access will be mailed to you shortly.");
	 * 
	 *               // send FTP folder details by email
	 ...
	 *               }
	 * 
	 *               private String generateEmail(String studyIdentifier, String
	 *               ftpFolder, User user) throws IsaTabException, DAOException {
	 * 
	 *               String privateFTPServer =
	 *               PropertiesUtil.getProperty("privateFTPServer");
	 *               String privateFTPRoot =
	 *               PropertiesUtil.getProperty("privateFTPRoot");
	 *               String privateFTPUser =
	 *               PropertiesUtil.getProperty("privateFTPUser");
	 *               String privateFTPPass =
	 *               PropertiesUtil.getProperty("privateFTPPass");
	 *               String linkFTPUploadDoc =
	 *               PropertiesUtil.getProperty("linkFTPUploadDoc");
	 *               // get the version of the app currently in use from the last
	 *               part of the path
	 *               String[] ftp_paths = privateFTPRoot.split("/");
	 *               String ftp_path = "/"+ftp_paths[ftp_paths.length - 1]+"/";
	 * 
	 *               // send FTP folder details by email
	 *               String subject = "Requested Study upload folder for " +
	 *               studyIdentifier;
	 *               StringBuilder body = new StringBuilder().append("We are happy
	 *               to inform you that your upload folder for study ")
	 *               .append("<b>").append(studyIdentifier).append("</b>")
	 *               .append(" has been successfully created and is now ready for
	 *               use. You can use either FTP or Aspera Client to upload your
	 *               study files.").append('\n').append('\n')
	 *               .append("<b>").append("Using FTP
	 *               Client:").append("</b>").append('\n')
	 *               .append('\t').append("user: ")
	 ...
	 *               .append('\n')
	 *               .append("Please, note that the remote folder needs to be
	 *               entirely typed, as the folder is not browsable. So use ")
	 *               .append("\"").append("<b>").append("cd
	 *               ").append(ftp_path).append(ftpFolder).append("</b>").append("\"").append("
	 *               to access your private folder.")
	 *               .append('\n')
	 *               .append('\n')
	 *               .append("<b>").append("Using Aspera
	 *               Client:").append("</b>").append('\n')
	 *               .append("You can also use the high-speed Aspera client to
	 *               upload with the same username and password listed above using
	 *               the command below.\n" +
	 *               "ascp -QT -P 33001 -L- -l 300M your_local_data_folder
	 *               mtblight@hx-fasp-1.ebi.ac.uk:").append(ftp_path).append(ftpFolder)
	 *               .append('\n')
	 *               .append('\n')
	 *               .append(" Detailed Instructions on data upload through
	 *               FTP/Aspera is available here: ").append(linkFTPUploadDoc)
	 *               .append('\n')
	 *               .append('\n');
	 *               logger.info("Sending upload folder details to " +
	 *               user.getEmail());
	 *               emailService.sendCreatedFTPFolderEmail(user.getEmail(),getSubmitterEmail(studyIdentifier,user),
	 *               subject, body.toString());
	 *               logger.info("Private upload folder details sent to user: {}, by
	 ...
	 * 
	 * @RequestMapping(value = "{studyIdentifier:" + METABOLIGHTS_ID_REG_EXP
	 *                       +"}/files/moveFilesfromFtpFolder", method=
	 *                       RequestMethod.POST)
	 * @ResponseBody
	 *               public RestResponse<Boolean>
	 *               moveFilesFromPrivateFtpFolder(@PathVariable("studyIdentifier")
	 ...
	 *              User user = getUser();
	 *              Study study =
	 *              getStudyDAO().getStudy(studyIdentifier,user.getApiToken());
	 ...
	 *              // move the files
	 *              String result =
	 *              FileUtil.moveFilesFromPrivateFtpFolder(fileNames, ftpFolder,
	 *              studyFolder);
	 *              RestResponse<Boolean> restResponse = new RestResponse<>();
	 *              restResponse.setContent(true);
	 *              restResponse.setMessage(result);
	 ...
	 * @PreAuthorize("hasRole('ROLE_SUPER_USER') or hasRole('ROLE_SUBMITTER')")
	 * @RequestMapping("{studyIdentifier:" + METABOLIGHTS_ID_REG_EXP
	 * +"}/files/privateFtpFolder")
	 * 
	 * @ResponseBody
	 *               public RestResponse<Boolean>
	 *               hasPrivateFtpFolder(@PathVariable("studyIdentifier") String
	 *               studyIdentifier)
	 *               throws DAOException, IsaTabException {
	 * 
	 *               String privateFTPRoot =
	 *               PropertiesUtil.getProperty("privateFTPRoot"); // ~/ftp_private/
	 * 
	 *               User user = getUser();
	 *               String ftpFolder = studyIdentifier.toLowerCase() + "-" +
	 *               getObfuscationCode(studyIdentifier, user);
	 * 
	 *               boolean result = FileUtil.getFtpFolder(privateFTPRoot +
	 ...
	 * @ResponseBody
	 *               public RestResponse<String[]>
	 *               getPrivateFtpFileList(@PathVariable("studyIdentifier") String
	 *               studyIdentifier)
	 *               throws DAOException, IsaTabException {
	 * 
	 *               String privateFTPRoot =
	 ...
	 * 
	 *               // read folder content
	 *               String[] fileNames = FileUtil.getFtpFolderList(privateFTPRoot +
	 *               File.separator + ftpFolder);
	 *               RestResponse<String[]> restResponse = new RestResponse<>();
	 *               restResponse.setContent(fileNames);
	 *               restResponse.setMessage("List of files in your private upload
	 *               folder");
	 * 
	 *               return restResponse;
	 *               }
	 ...
	 * 
	 * @RequestMapping(value = "{studyIdentifier:" + METABOLIGHTS_ID_REG_EXP
	 *                       +"}/files/deleteFilesfromFtpFolder", method=
	 *                       RequestMethod.POST)
	 * @ResponseBody
	 *               public RestResponse<Boolean>
	 *               deleteFilesFromPrivateFtpFolder(@PathVariable("studyIdentifier")
	 *               String studyIdentifier,
	 * @RequestBody List<String> fileNames)
	 *              throws DAOException, IsaTabException, IndexingFailureException,
	 *              IOException {
	 * 
	 *              User user = getUser();
	 *              logger.info("User {} requested to delete files from study {}
	 *              private upload folder.", user.getFullName(), studyIdentifier);
	 * 
	 ...
	 *              // delete the files
	 *              String result =
	 *              FileUtil.deleteFilesFromPrivateFtpFolder(fileNames, ftpFolder);
	 * 
	 *              // compose a response
	 *              RestResponse<Boolean> restResponse = new RestResponse<>();
	 *              restResponse.setContent(true);
	...
	/**
	 * Returns list of Metabolites identified in the given Metabolights Study -
	 * MTBLSX
	 *
	 * @param studyIdentifier
	...
	@ResponseBody
	public RestResponse<Study> getIdentifiedMetabolites(@PathVariable("studyIdentifier") String studyIdentifier)
			throws DAOException {
	...
	@ResponseBody
	public RestResponse<Study> getMetabolitesInchi(@PathVariable("studyIdentifier") String studyIdentifier)
			throws DAOException {
	...
	@RequestMapping(value = "{studyIdentifier:" + METABOLIGHTS_ID_REG_EXP
			+ "}/getPermissions", method = RequestMethod.POST)
	@ResponseBody
	public RestResponse<String> getPermissions(@PathVariable("studyIdentifier") String studyIdentifier,
		...
		try {
			...
			String add_study_info = ", \"studyLocation\": \"" + studyLocation + "\", \"obfuscationCode\": \""
					+ obfuscationCode + "\", \"releaseDate\": \"" + study.getStudyPublicReleaseDate()
					+ "\", \"submissionDate\": \"" + study.getStudySubmissionDate() + "\", \"studyStatus\": \""
					+ studyStatus + "\"";
	...
	@ResponseBody
	public RestResponse getMetExploreMappingData(@PathVariable("studyIdentifier") String studyIdentifier)
			throws DAOException {
		...
		String MetExploreJSONFileName = PropertiesUtil.getProperty("studiesLocation") + studyIdentifier + File.separator
	...
	@ResponseBody
	public RestResponse<Study> updateStudyMetaboliteMapping(@PathVariable("studyIdentifier") String studyIdentifier)
			throws DAOException {
	...
	private boolean mapCompound(String chebiId, String studyIdentifier, String species) {
		...
		try {
			...
			// Check if we have already the Metabolite (since querying the WS is what takes
			// more...)
		...
		try {
			...
			if (study == null) {
				...
				for (Assay assay : study.getAssays()) {
					...
					for (MetaboliteAssignmentLine mal : assay.getMetaboliteAssignment()
							.getMetaboliteAssignmentLines()) {
	...
	public RestResponse<String> sendValidationsStatusMail(@PathVariable("studyIdentifier") String studyIdentifier)
		...
		User user = getUser();
		logger.info("User {} has requested a validations status report to be sent by mail for the study {}",
				user.getUserName(), studyIdentifier);
	...
	public RestResponse<String> createEmptyStudy(HttpServletRequest request) throws DAOException {
		...
		try {
			...
			logger.info("createEmptyStudy: User {} has requested a new empty study, using token {}", user.getEmail(),
					token);
	...
	// @ResponseBody
	// public RestResponse<String> reindexStudyOnToken(HttpServletRequest request)
	// throws DAOException {
	...
	// try {
	// user = usi.lookupByToken(token);
	// logger.info("reindexStudyOnToken: User {} has requested reindexing study {}",
	// token, study_id);
	// } catch (Exception e) {
	// logger.error ("Not able to authenticate the user for 'reindexStudyOnToken'
	// mapping ");
	// response.setMessage("Not able to authenticate the user for
	// 'reindexStudyOnToken' mapping.");
	// response.setErr(e);
	// return response;
	// }
